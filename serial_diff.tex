...
\DIFadd{Here, changes in reproduction number can be modeled as a product of the basic reproduction number $\Ro$, proportion susceptible $S(t)$, and some intervention function $M(t)$, which can either decrease and/or increase in time: $\RR(t) = \Ro S(t) M(t)$;
\mbox{%DIFAUXCMD
...
Since we are interested in the initial epidemic growth phase (i.e., linking $r$ to }\RR\DIFadd{), the impact of intervention and susceptible depletion is expected to be negligible.
