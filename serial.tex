...
\djde{deleted ``At the individual-level, both forward and backward
  ...
  and backward delay \emph{distributions}:'' and ended up rewriting
...
(B) Changes in the estimate of \RR based on the observed serial intervals over time.
(C) Schematic representation of line list data rearranged by symptom onset date of infectors.
(B) Changes in the estimate of \RR based on the cohort-averaged serial intervals over time.
}
\label{fig:obsrR}
\end{figure}
...
\fref{obsrR}B demonstrates how the effect of right-censoring in the observed serial intervals translates to the underestimation of \RR in our stochastic simulations.
...
Then, we can compare how estimates of the mean serial interval as well as \RR change as we incorporate more recent cohorts into the analysis;
...
This approach allows us to detect dynamical changes in the forward serial-interval distributions and its effect on the estimates of \RR.
