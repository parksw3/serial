...
When we measure an epidemiological time delay within an infected individual (e.g., the incubation period), the primary event \jd{is the event that always or usually} always occurs before the secondary event.
...
\jd{Thinking a lot about notation. I kind of want to make it easier for people to follow the logic by giving $\ell$ and $C$ different names. Like maybe P(t) for the primary-event cohort and D(t) for the secondary-event cohort? And then $\pi$ and $\delta$ instead of $\ell$?}
Likewise, we define the backward delay distribution $b_\ell(\tau)$ for a secondary cohort $\ell$:
The backward delay distribution describes the time delays between a primary and secondary host given that the secondary event occurred at time $\ell$.
It follows that:
\begin{equation}
b_\ell(\tau) \propto C(\ell-\tau) f_{\ell-\tau}(\tau)
\end{equation}
\jd{Brilliant, but would be easier to follow if more symmetric. Maybe start from the idea that we can add up events in either direction. Using the proposed notation, I might start from $D(\delta) b_\delta(\delta-\pi)  = P(\pi) f_\pi(\delta-\pi)$}
where $C(\ell)$ is the size of the primary cohort $\ell$.
Therefore, changes in the backward delay distribution depend on the changes in cohort size $C(\ell)$ (therefore incidence of infection) as well as changes in the forward delay distribution.
...
Given that an infector became symptomatic at time $\ell$, we first go backward in time by asking when the infector was infected, and then go forward in time by asking first when the infectee was infected and then when the infectee became symptomatic;
this defines the forward serial interval.
In \fref{diagram}A, we see that $x_0$ represents the backward incubation period of infectors who became symptomatic at time $\ell$; 
...
\jd{This needs more unpacking of what is conditional and why. We should spend some words talking about the fact that we expect forward internal intervals to be independent of dynamics (but not necessarily biology, I guess).}
...
\jd{We should clearly state that we're always going to assume that intervals don't change intrinsically (or however we say it, the point is we're only going to think about changes that are driven by dynamics).} 
\begin{equation}
...
\jd{So obviously I was wrong about $k$; I get it now. I feel like the way to make this clear is similar to what DC and I did. First calculate the density of \emph{events}: this is the same numerator for the forward and backward distributions, the number of intervals that start at the beginning and end at the end; it's only the denominators that differ. I'm struggling again with \Rc: what matters is the number of susceptibles when I actually infected the infectee; using \Rc\ seems like we're averaging over something we shouldn't be averaging over.}
\begin{equation}
...
\jd{An alternative approach that I'm pretty sure produces a slightly different answer. I'm going to write it in a mixed notation:}
The number of SIs that go from $\delta_1$ to $\delta_2$ is an integral over all of the possibilities for the infection times.
\begin{equation}
	N(\delta_1, \delta_2) = \int \int d\alpha_1 d\alpha_2
	i(\alpha_1) h(\delta_1-\alpha_1, \alpha_2-\alpha_1)
	S(\alpha_2)\theta(\delta_2-\alpha_2)
\end{equation}
To get the forward SI, we substitute $\delta_2 = \delta_1+\tau$ and normalize. And a similar trick (from the same starting point) to get the backward SI.
