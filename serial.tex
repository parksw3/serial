...
The multivariate log-normal distribution is parameterized based on pameter estimates for COVID-19.
We construct the forward serial-interval distribution during the exponential growth period as follows:
\begin{equation}
S_i = -B_i + (G_i|B_i) + I_i,
\end{equation}
...
\end{equation}
We compare this estimate with a naive estimate of $\mathcal R$ that assumes that the backward and the forward incubation periods are identically distributed:
\begin{equation}
