...
\begin{equation}
f_{0}(\tau) = \frac{\int_{-\infty}^{0} \int_{\alpha_1}^{\tau} \exp(r \alpha_1) h(-\alpha_1, \alpha_2 - \alpha_1) \idist(\tau - \alpha_2) \, \mathrm{d}\alpha_2\,\mathrm{d}\alpha_1}{\int_{-\infty}^\infty \int_{-\infty}^{0} \int_{\alpha_1}^{\tau'} \exp(r \alpha_1) h(-\alpha_1, \alpha_2 - \alpha_1) \idist(\tau - \alpha_2) \, \mathrm{d}\alpha_2\,\mathrm{d}\alpha_1 \dtau'},
\label{eq:fdist}
\end{equation}
where $h$ is the joint probability distribution describing the the intrinsic generation-interval distribution $g$ and the intrinsic incubation period distribution $\idist$ (see \eref{marginal} in the main text).
For brevity, we denote the constant of proportionality using $\phi$ such that
\begin{equation}
...
\end{equation}
We substitute $z(\sinf)$ into \eref{newforward} to obtain:
\begin{equation}
f_{0}(\tau) = \frac{1}{\phi} \int_{-\infty}^{\tau} z(\sinf) \idist(\tau - \sinf) \,\mathrm{d}\sinf\,,.
\end{equation}
By normalizing $z(\sinf)$ such that $\hat{z}(\sinf) = z(\sinf)/\left[\int_{-\infty}^\infty z(x) \dx\right]$,
we can now express the initial forward serial-interval distribution $f_0$ as a convolution of $\hat{z}$ and $\idist$:
\begin{equation}
f_{0}(\tau) = \frac{1}{\phi} \int_{-\infty}^{\tau} \hat{z}(\sinf) \idist(\tau - \sinf)\, \mathrm{d}\sinf\,.
\end{equation}
Since the right hand side of \eref{ptarget} is also a moment-generating function of $f_0$, we can express it as the product of the moment-generation functions of $\hat{z}$ and $\idist$:
\begin{equation}
...
\end{align}
Therefore, the initial forward serial-interval distribution and the intrinsic generation-interval distribution give the same estimates of \Ro from $r$.
...
\end{equation}
We compare this with an estimate of \Ro based on the intrinsic serial-interval distribution which has the same mean as the intrinsic geeneration-interval distribution \citep{svensson2007note,klinkenberg2011correlation,champredon2018equivalence, britton2019estimation}:
\begin{equation}
