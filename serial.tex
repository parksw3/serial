...
The density of serial intervals between time $\psymp$ and $\ssymp$,
...
\label{eq:fserial}
\end{equation}
The denominator, $\int_{-\infty}^\infty \total(\psymp, \psymp+\tau') \dtau'$, then corresponds to the total number of symptomatic cases generated by primary cases who developed at time \psymp;
dividing it by the incidence of symptomatic cases (also infected by symptomatic cases) at time \psymp, $j(\psymp) = \int_{-\infty}^\infty \total(\psymp-\tau', \psymp) \dtau'$, we obtain the serial reproduction number:
\begin{equation}
\Rs(\psymp) = \frac{\int_{-\infty}^\infty \total(\psymp, \psymp+\tau') \dtau'}{j(\psymp)},
\end{equation}
which we define as the average number of symptomatic cases generated by individuals who developed symptoms at time \psymp (and were also infected by sympmtomatic cases).
Combining the forward serial-interval distribution with the serial reproduction number allows us to express the renewal process between symptomatic cases:
\begin{equation}
j(t) = \int_{-\infty}^\infty \Rs(t-\tau) j(t-\tau) f_{t-\tau}(\tau) \dtau.
\label{eq:jrenew}
\end{equation}
In Supplementary Materials, we show that estimating the serial reproduction number requires the backward serial-interval distribution $b_\ssymp(\tau)$, which is given by the density of intervals of length $\tau$ ending at \ssymp, relative to the total number of serial intervals ending at \ssymp: 
\begin{equation}
b_\ssymp(\tau) = 
\frac{\total(\ssymp-\tau, \ssymp)}{\int_{-\infty}^\infty \total(\ssymp-\tau', \ssymp) \dtau'}.
\label{eq:bserial}
\end{equation}
This framework allows us to understand changes in the realized serial intervals for any epidemic model and link serial-interval distributions with the renewal process.
...
nonetheless, this form has been widely used in the literature and has been successfully applied in modeling the current COVID-19 pandemic.
...
Throughout the paper, we allow for presymptomatic transmission to match the transmission dynamics of COVID-19.a

\subsection{Model parameterization}

The dynamics of the serial-interval distribution depends heavily on the joint distribution between incubation periods and generation intervals.
Here, we use a bivariate lognormal distribution to model the joint probability distribution of the intrinsic incubation periods and the intrinsic generation intervals while allowing for the possibility that they might be correlated.
...
The marginal lognormal distributions are parameterized based on parameter estimates for COVID-19 (Table 1).
