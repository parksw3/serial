...
Finally, the forward serial-interval distribution $f_\pt(\tau)$ is proportional to $\total (\pt, \pt+\tau)$ because it is conditional on the cohort of infectors who became symptomatic at time $\pt$:
\begin{equation}
...
The mean forward generation interval decreases slightly as the epidemic progresses due to susceptible depletion (\fref{epi}C; \cite{kenah2008generation, champredon2015intrinsic}).
...
Serial intervals are typically measured from line-list data (\fref{obsrR}A).
...
The decrease in the mean forward serial interval is likely to be cause by increase in the mean backward serial intervals as well as increase in mitigation efforts.
