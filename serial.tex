...
Then, for \swp{an average individual in} primary cohort at time $\psymp$, we can define the expected time distribution between primary and secondary events. 
We refer to this distribution as the forward delay distribution and denote it as $f_\psymp(\tau)$.
\swp{Since the forward delay distribution is defined for an average individual, we do not assume that all individuals who experience the primary event must experience the secondary event.}
...
secondary cohorts then we \swp{can express the total density of intervals between time $\psymp$ and $\ssymp$ as follows:}
\begin{equation}
W(\psymp) \psize(\psymp) f_\psymp(\tau) = \ssize(\ssymp) b_\ssymp(\tau) \,,
\label{eq:match}
\end{equation}
where $W(\psymp)$, the ``weight'' of the primary cohort, represents the average number of forward intervals that an \swp{average} individual in cohort $\psize(\psymp)$ produces over the course of their infection.
...
this equation is the probability density that an \swp{average} individual
...
the time-varying primary cohort size at the earlier time $\psize(\ssymp-\tau)$ (in this case, the number of individuals became infected at time $\ssymp-\tau$),
and the forward delay distribution $f_{\ssymp-\tau}(\tau)$ (in this case, the probability density that \swp{an average individual} who was infected at time $\ssymp-\tau$ became symptomatic at time $\ssymp$).
...
\jd{Am I right that the main thing going on here is the decrease in $r(t)$, which should be a robust feature of outbreaks in general? Can we explain that?}
\swp{You mean decrease in $i(t)$. I had explained that previously (see what's commented out above; Since the increase ...) but DC complained that he didn't like it. I'd be OK with keeping it in.}
