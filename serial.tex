...
This conceptual framework clearly demonstrates that the distributions of $x_0$, $\gtime$, and $x_1$ (and therefore the distributions of realized serial intervals) depend on the reference cohort and the corresponding cohort time.
...
\begin{equation}
\hat{\idist}(m \Delta t) = \frac{\idist(m \Delta t)}{\sum_{i=1}^\ell \idist(m \Delta t)}, \quad m=1, \dots, \ell,
\end{equation}
...
\begin{equation}
\hat{h}(m \Delta t, \Delta t n) = \hat{\idist}(m \Delta t)\hat{\gdist}(\Delta t n), \quad m,n=1, \dots, \ell.
\end{equation}
