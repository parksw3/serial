...
\jd{It's a shame not to mention the idea that serial intervals are wider, and should therefore predict smaller \RR. That would also help with understanding the resolution. Or maybe we get to these ideas later? It helps with the paradox framing, but I guess you really don't like that.}
...
\jd{I feel that $R_c$ should group with h (i.e., after i()), for comprehension.}
\begin{equation}
...
\end{equation}
Rather than numerically integrating over closed forms of $g$, $f_0$, and $q$ to estimate $\Ro$, we use simulation-based approaches for simplicity (Supplementary Materials). Our parameter values are shown in the Table.
