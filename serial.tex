...
However, the intrinsic time distributions are not always equivalent to the corresponding realized time distributions at the population level (i.e., distribution of time between primary and secondary events that occur during an epidemic).
For example, an infectious contact may not result in infection if the contacted individual is not susceptible;
...
For example, when we are measuring incubation periods, a \textbf{p}rimary cohort consists of all individuals who became infected at time $\psymp$.
...
Then, for each primary cohort $\psymp$, we can define the expected time distribution between primary and secondary events.
We refer to this distribution as the forward delay distribution and denote it as $f_\psymp(\tau)$.

Likewise, we define the backward delay distribution $b_\ssymp(\tau)$ for a \textbf{s}econdary cohort at time $\ssymp$:
...
must yield identical measurement (e.g., the length of the incubation
...
Backward delay distributions depend on changes in the primary cohort size over time due to conditionality of observations (i.e., they are measured from a cohort of individuals whose secondary events occurred at160the same time).
...
To correctly link serial intervals to the renewal process between cases based on symptom onset dates, we must use the forward serial interval (i.e., use the perspective of a cohort of infectors that share the same symptom onset time). 
...
$\gtime$ follows the forward generation-interval distribution of the cohort of infector who came infected at time $\psymp - x_0$;
...
Likewise, we can define the backward serial interval distribution for a secondary cohort $\ssymp$ (\fref{diagram}C).
This conceptual framework clearly demonstrates that the distributions of $x_0$, $\ssymp$, and $x_1$ (and therefore the distributions of serial intervals) depend on the reference cohort and the corresponding cohort time.

% \jd{Could we skip this \P?} 
% Given that an infectee became symptomatic at time $\ssymp$, we have to first go backward in time by asking when the infectee became infected and when the infector became infected; 
% then, we have to go forward in time by asking when the infector became symptomatic.
% In this case, $x_1$ follows the backward incubation period distribution of the cohort of infectees who became symptomatic at time $\ssymp$;
% $\gtime$ follows the backward generation-interval distribution of the cohort of infectees who became infected at time $\ssymp-x_1$;
% and $x_0$ follows the forward incubation period distribution of the cohort of infectors who became infected at time $\ssymp-x_1-\gtime$.
...
\times
\underbrace{k_{\sinf}(\ssymp - \sinf)}_{\substack{\text{marginal density of} \\ \text{the forward incubation} \\ \text{period } \ssymp-\sinf \\ \text{(of infectees)}}},
\end{equation}
...
\begin{equation}
\total (\psymp,\ssymp) = \int_{-\infty}^{\psymp} \int_{\pinf}^{\ssymp} \Rc (\pinf) i(\pinf) h_{\pinf}(\psymp-\pinf, \sinf - \pinf) k_{\sinf}(\ssymp - \sinf) \, \mathrm{d}\sinf\,\mathrm{d}\pinf.
\end{equation}
...
\begin{equation}
f_{0}(\tau) \propto \int_{-\infty}^{0} \int_{\pinf}^{\tau} \exp(r \pinf) h(-\pinf, \sinf - \pinf) k(\tau - \sinf) \, \mathrm{d}\sinf\,\mathrm{d}\pinf.
\end{equation}
In the Appendix, we provide a mathematical proof of this relationship.
\jd{$k$ for marginal density is really slowing me down (since Champredon uses k for Rg). Maybe that's just my problem, but maybe we can think of a better letter.}
...
\jd{I thought you were doing disease-model simulations (like DC does). I'm surprised we model the distribution of SIs instead of letting it emerge. I think I agree that it's still convincing because of the rR link, but it would be better to do the full-simulation way if that's practical.}
...
When an outbreak is growing exponentially, forward serial intervals have higher means (\fref{rR}C) and squared coefficients of variation (\fref{rR}D) than the intrinsic generation-interval distribution.
...
\jd{I don't see orange lines, and the location of the dashed line is not explained. I guess you use the forward value at time 0? Would the intrinsic value be better?}
...
\jd{This is hard to follow. We still haven't said much about why we're interested in backward intervals. And it's hard to think clearly about the assumption of incubation periods being independent of generation intervals. The dynamics are a potential complication. Presumably we mean that at any given time, we are assuming that these things are independent across individuals.}
...
Since both forward incubation period (\fref{epi}B) and generation-interval (\fref{epi}C) distributions remain roughly constant, changes in the forward serial-interval distributions are predominantly driven by changes in the backward incubation period distributions, whose mean increases over time (\fref{epi}D). \jd{Are you saying that this is true in general, or for these parameters? Have we said enough about these parameters to make this clear to others?}
...
\jd{Bringing intervention in here is potentially confusing. If you want to do it, maybe try a bit more lead in?} 
...
(B) Estimates of \Ro based on all observed serial intervals completed by a given time. \jd{Is this accurate?}
...
This approach is analogous to averaging over a set of forward intervals, just as using all information up to a certain time is analogous to averaging over a set of backward intervals.
(\fref{obsrR}D).
...
\jd{Is censoring relevant to negative values as well? We might not have (or can't?) observe events before a certain time? It's a bit confusing.}
The decrease in the mean forward serial interval was likely caused by increase in the mean backward serial intervals \jd{incubation periods? is it obvious to the reader why these are increasing?} as well as intervention efforts which reduce generation intervals by preventing infections once cases are identified.
...
Generation- and serial-interval distributions determine the time scale of disease transmission, and are therefore critical to dynamical modeling of infectious outbreaks.
...
\jd{I'm not in love with Goldstein as a practical approach, even though it was more or less my idea! I'd like to learn more about Flaxman.}
...
however, our study demonstrates that early serial interval data is required to estimate \RR and using up-to-date serial interval data can, in fact, underestimate \RR.
\jd{I'm confused by this. We should be working towards methods that don't get the wrong answer just because you give them more information. Also, if \RR is assumed to be time-varying then of course you want to be up-to-date, no?}
Moreover, \cite{tempvar} recently analyzed case reports of COVID-19 from over 30 countries to estimate their time-varying reproduction numbers but relied on a [fixed] serial-interval distribution;
...
\jd{Isn't it more interesting for our purposes whether they appropriately considered the \emph{forward} interval?}
...
Our study also underlines the fact that the serial-interval distribution depends not only on the generation-interval and incubation-period distributions, but also on their correlations.
...
\jd{Need to be clearer what source of difference is relevant here.}
...
We assumed that all individuals develop symptoms and that the the entire transmission process (e.g., direction of transmission, symptom onset dates, infection dates, etc.) are observable. \jd{I don't think we quite assumed that infection dates are observable, we can discuss.}
...
For example, symptom onset dates cannot be used as a reliable proxy for determining the direction of transmission as infectees may develop symptoms before their infectors. \jd{Not an example, and not really something we don't consider.}
...
\jd{Maybe leave out the susceptible depletion and just jump straight to the sunky/trace stuff? }
