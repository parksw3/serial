...
\label{eq:match}
\end{equation}
where $W(\psymp)$, the ``weight'' of the primary cohort $\psize(\psymp)$, represents the average number of forward intervals that an individual in cohort $\psize(\psymp)$ produces over the course of their infection.
...
When we measure between-individual delays, we expect $W(\psymp)$ to change throughout an epidemic because individuals infected in the beginning of an epidemic will infect more than one individual, on average, over the course of their infection, whereas those infected later will infect less than one individual, on average.
