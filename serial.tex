...
length $\tau$. 
From the right hand side, we see that this probability density depends on both the time-varying primary cohort size at the earlier time $\psize(\ssymp-\tau)$ (in this case, the number of individuals became infected at time $\ssymp-\tau$) and the forward delay distribution $f_{\ssymp-\tau}(\tau)$ (in this case, the probability density that someone who was infected at time $\ssymp-\tau$ became symptomatic at time $\ssymp$).
