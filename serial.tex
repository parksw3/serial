...
At the individual level, we can define the expected time distribution between a primary and a secondary event --- we refer to this distribution as the intrinsic distribution.
For example, the intrinsic incubation period distribution describes the expected time from infection to symptom onset of an individual.
Likewise, the intrinsic generation-interval distribution describes the expected time from infection of a primary case to infection of a secondary case in a fully susceptible population; 
since this is a expected time distribution, it does not take into account dynamical factors such as susceptible depletion or intervention efforts.
However, the intrinsic time distributions are not always equivalent to realized time distributions at the population level.
...
(A) Cumulative incidence over time. \jd{Would it be easier for people to think about dynamics if we show incidence instead?}
