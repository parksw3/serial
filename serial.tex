...
\newcommand{\psymp}{\ensuremath{\rho_1}} %% primary symptom time
\newcommand{\ssymp}{\ensuremath{\rho_2}} %% secondary symptom time
...
\jd{I think it would be nice to have intrinsic incubation periods, and to make the point somewhere that because of causality we expect the forward to equal intrinsic incubation. I also wonder if we should skip the }
...
\jd{Could we skip this \P?} Likewise, we can define the backward serial interval distribution for a secondary cohort $\ssymp$ (\fref{diagram}C).
...
\jd{This exposition is repetitive.}
To calculate realized serial-interval distributions, we begin by modeling $\total(\psymp,\ssymp)$: the total number of serial intervals that start (when infectors develop symptoms) at time $\psymp$ and end at time $\ssymp$).
The number of serial intervals between time $\psymp$ and $\ssymp$ given that the infectors became infected at time $\pinf$ and the infectees became infected at time $\sinf$ is can be then expressed as a product of (i) incidence $i$ at time $\pinf$, (ii) their case reproduction number $\Rc(\pinf)$ (i.e., the average number of secondary cases caused by a primary case infected at time $\pinf$ over the course of their infection, \cite{fraser2007estimating}), (iii) the density of forward generation interval between $\pinf$ and $\sinf$, and (iv) the densities of forward incubation periods between $\pinf$ and $\psymp$ (incubation periods of infectors) as well as $\sinf$ and $\ssymp$ (incubation periods of infectees):
\begin{equation}
\underbrace{i(\pinf)}_{\text{incidence}} 
\times 
\underbrace{\Rc (\pinf)}_{\substack{\text{case} \\ \text{reproduction} \\ \text{number}}} 
\times 
\underbrace{h_{\pinf}(\psymp-\pinf, \sinf - \pinf)}_{\substack{\text{joint density of} \\ \text{the forward incubation} \\ \text{period and the forward} \\ \text{generation interval}\\ \text{(of infectors)}}}
\times
\underbrace{k_{\sinf}(\ssymp - \sinf)}_{\substack{\text{marginal density of} \\ \text{the forward incubation} \\ \text{period (of infectees)}}},
\end{equation}
\jd{We've already described all of these things twice. Could we just stop here? Including, skip the integral below.} where $h_{\pinf}(\psymp-\pinf, \sinf - \pinf)$ is the joint probability distribution describing the forward incubation period $\psymp-\pinf$ and the forward generation interval $\sinf - \pinf$ of a cohort of individuals infected at time $\pinf$ (in this case, the infectors), and $k_{\sinf}(\ssymp-\sinf)$ is the marginal probability distribution of $h_{\sinf}$ describing the forward incubation period $\ssymp-\sinf$ of a cohort of individuals infected at time $\sinf$ (in this case, the infectees):
\begin{equation}
k_{\sinf}(\tau) = \int_0^\infty h_{\sinf}(\tau, \sigma) \dsigma.
\end{equation}
...
Therefore, we expect the forward serial-interval distribution during the exponential growth phase --- which we refer to as the \emph{initial} forward serial-interval distribution $f_0$ --- to link the same values of $r$ and \Ro as the intrinsic generation-interval distribution:
\jd{I don't want to talk about the ``link'' here, because that to me implies the functional link, not just linking a single pair of values.}
\begin{equation}
