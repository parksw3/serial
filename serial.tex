...
Then, it is clear that $x_0$ represents the backward incubation period for secondary cohort $\ell$; 
$\sigma$ represents the forward generation interval for a primary cohort $\ell - x_0$ conditional on the incubation period $x_0$ of the infector;
and $x_1$ represents the forward incubation period for a primary cohort $\ell - x_0 + \sigma$.
...
Finally, $\mathcal R_c (\ell - x_0)$ is the cohort reproduction number, which is the average secondary number of cases caused by an individual infected at time $\ell - x_0$.
The cohort reproduction number plays a critical role in defining the forward serial interval because serial intervals can only be measured after the disease has been transmitted; 
we revisit this point in the Results section.
...
In this case, $x_1$ and $\sigma$ represent the backward incubation period and generation-interval distributions, respectively, and $x_0$ represents the forward incubation period distribution.
...
We further tease apart factors that affect the forward serial interval distribution, including the cohort reproduction number, as mentioned previously.
