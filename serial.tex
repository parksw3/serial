...
Similarly, the serial interval is defined as the time between when an infector and an infectee become \emph{symptomatic}.
While serial intervals are similar to generation intervals, previous studies noted that using serial intervals can give different estimates of $\mathcal R$ and have noted that in many contexts serial intervals are expected to have larger variances than generation intervals (but the same mean).
...
>>>>>>> 7aeb8d1c6801e19b0d6643849abadaf2ca4817f6
...
We develop a new framework for understanding intervals and show that the forward serial-interval distribution gives the correct value of $\mathcal R$.
We also show that \jd{Come back to this. Not just over time, right?} However, failing to account for changes in the observed serial-interval distributions over time can severely bias the estimate of $\mathcal R$.
=======
...
We first begin by describing a general framework for characterizing a distribution of time delays between any two epidemiological events;
...
Orange lines represent the results of 10 stochastic simulations.
Orange points are calculated by averaging across 10 stochastic simulations.
