...
Instead, the forward serial-interval distribution depends on $r$: 
When an epidemic is growing exponenially, infectors that develop symptoms on a given day will have shorter incubation periods, on average, than their infectees, thereby affecting the realized serial-interval distribution.
We provide a heuristic method for addressing biases that can arise from failing to account for changes in serial intervals across cohorts; we apply the method to estimation of \RR for the COVID-19 outbreak in China, outside Hubei province, using serial-interval data (collected between January  21--February  8,  2020). Our analysis shows that using incorrectly defined serial intervals can bias estimates of initial \RR by a factor of 2.0--2.6 in the context of COVID-19.
...
Most methods of \RR estimation require a detailed knowledge of the life history along with reliable incidence data.
Alternatively, near the beginning of an epidemic, \RR can be estimated from the population-level exponential growth rate $r$.
...
These studies are exampels of many---indeed, it is a common practice to use the serial and generation intervals interchangeably.
...
For example, in our analysis of the COVID-19 serial intervals from China, we find that the original \Ro estimates based on aggregated serial-interval data underestimated \Ro by a factor of 2.0--2.6.
...
Thus, the mean backward interval will be always shorter than the mean forward interval as long as $r > 0$---even for different epidemics of the same disease, we expect to observe shorter backward intervals within a fast-growing epidemic (high $r$), all else being equal.
...
In particular, we see that the initial forward serial-interval distribution, $f_0(\tau)$, provides a correct link between the exponential growth rate $r$ and the initial serial reproduction number $\Rs(0)$ by substituting $j(t) \approx j(0) \exp(rt)$, $\Rs(t) \approx \Rs(0)$, and $f_{t-\tau}(\tau) \approx f_0(\tau)$:
\begin{equation}
\frac{1}{\Rs(0)} = \int_{-\infty}^\infty \exp(-r\tau) f_{0}(\tau) \mathrm{d} \tau.
\label{eq:Rforward0}
\end{equation}
\swp{OK to add this here? Do we needs a concluding sentence for this section?}
...
Although we do not consider non-pharmaceutical interventions or behavioral changes explicitly, the effect of susceptible depletion is mathematically equivalent to the effect of interventions that reduce transmission rate, and therefore \RR(t).
\swp{JSW: Should say we do not consider NPI and/or behavior explicitly. I'm not sure that it's all exactly equivalent; because then R0 would vary and I don't see what the last part of this sentence really means anyway. Also, I think Sang Woo would argue this is true even when peaks/declines occur in the absence of susceptible depletion which is the most likely thing happening for COvid-19. Write this sentence carefully!}
...
While it is straightforward to see this relationship from the renewal equation (see \eref{Rforward0}),
...
\bmb{what conclusion specifically? or do you mean this ``formulation'' is robust? Do you need to be more explicit about the inferential status of ``Therefore, we expect \ldots'' above? Is }
\swp{I tried to deal with the first one. Not sure what he means by the second question?}
...
Everywhere else in this paper, however, we do not assume that the incubation and exposed periods are equivalent---instead, we allow for presymptomatic transmission in the model in order to reflect the transmission dynamics of COVID-19.
...
\swp{JSW is confused because exp(1.54)=4.66 but that's because those are parameters of the not log of the mean but rather mean of the normal. I tried to clarify that. He also wanted me to add $\mu_I$ and $\sigma$ but I think that's confusing because it won't be $\exp(\mu)$...}
}
\end{table}
