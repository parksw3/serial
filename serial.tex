...
However, recent studies suggest that making this substitution can lead to significant changes when using $r$ to estimate \RR.
Here we explore the differences among these intervals, how they vary over the course of an epidemic, and the implications for estimating epidemic parameters.
We show that \emph{forward-looking} serial intervals, which begin in a certain period, correctly describe the renewal process of symptomatic cases and therefore reliably link \RR with $r$.
...
The forward-looking intervals depend on $r$:
During the exponential growth phase of an epidemic, infectors that develop symptoms on a given day will have shorter incubation periods, on average, than their infectees, changing the serial-interval distribution in complex ways that depend on 
...
Our analysis shows that using improperly defined serial intervals can bias estimates of initial \RR by up to a factor of 2.6 in the context of COVID-19.
