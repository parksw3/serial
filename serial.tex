...
Here, changes in reproduction number can be modeled as a product of the basic reproduction number $\Ro$, proportion susceptible $S(t)$, and some intervention function $M(t)$, which can either decrease and/or increase in time: $\RR(t) = \Ro S(t) M(t)$;
...
Since we are interested in the initial epidemic growth phase (i.e., linking $r$ to \RR), the impact of intervention and susceptible depletion is expected to be negligible.
In addition, the impact of such intervention $M$ that reduces $\RR(t)$ monotonically over time will be identical to the impact of susceptible depletion under this modeling framework. 
...
They therefore are not expected to explain all the variation in forward serial intervals, since these additionally depend on both the backward incubation period of the infector and (iii) the forward incubation period of the infectee (\fref{diagram}B).
