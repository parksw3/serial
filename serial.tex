...
Generation and serial intervals are under-appreciated yet important quantities for characterizing the outset of an outbreak.
...
The generation- and serial-interval distributions are key quantities in outbreak analyses.
Recent theoretical studies suggest that two distributions give different estimates of \RR;
however, both intervals, by definition, can equally describe the disease transmission at the individual level.
...
We then apply our framework to serial interval data from the COVID-19 epidemic.
While our study supports the use of serial-interval distributions in estimating \RR, it also reveals several gaps in the current understanding and applications of the serial-interval distributions.
...
the primary event refers to the event that occurs in the infector; again some of these delays are always positive (the infector is always infected before the infectee) and some are not (it is possible for the infectee to report symptoms before the infector). 
