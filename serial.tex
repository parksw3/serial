...
\times 
\underbrace{h_{\alpha_1}(\pt-\alpha_1, \alpha_2 - \alpha_1)}_{\substack{\text{joint density of} \\ \text{the forward incubation} \\ \text{period and the fowrard} \\ \text{generation interval}}}
\times
\underbrace{k_{\alpha_2}(\st - \alpha_2)}_{\substack{\text{marginal density of} \\ \text{the forward incubation} \\ \text{period}}},
\end{equation}
...
Finally, the forward serial-interval distribution is proportional to $\total (\pt, \pt+\tau)$ because it is conditional on the cohort of infectors who become symptomatic at time $\pt$. Substituting $x=\pt-\alpha_1$ and $\gtime=\alpha_2-\alpha_1$, we have:
\begin{equation}
f_\pt(\tau) \propto \int_{0}^{\infty} \int_{0}^{\max(0,x+\tau)} \Rc (\pt-x) i(\pt-x) h_{\pt-x}(x, \gtime) k_{\pt-x+\gtime}(x-\gtime+\tau) \dsigma \dx
\end{equation}
Likewise, the backward serial-interval distribution is proportional to $\total (\st-\tau, \st)$ because it is conditional on the cohort of infectees who become symptomatic at time $\st$.  
