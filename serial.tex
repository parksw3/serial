...
\begin{equation}
\int_{-\infty}^\infty \exp(-r\sinf) f_{0}(\tau)\dtau = \int_{-\infty}^\infty \exp(-r\sinf) \int_{\max{(0,-\tau)}}^{\infty} \exp(-r\pinf) h(\pinf, \tau + \pinf)\, \mathrm{d}\pinf\,\mathrm{d}\sinf\,.
\label{eq:newtarget2}
\end{equation}
...
&=\int_{-\infty}^{\infty} \exp(-r\sinf) \int_{\max(0, -\sinf)}^\infty \exp(- r \pinf) h(\pinf, \tau+\pinf) \,\mathrm{d} \pinf \,\mathrm{d}\sinf \\
